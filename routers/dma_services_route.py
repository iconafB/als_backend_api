...
from schemas.dma_tracker_schema import (CreateDMARecordResponse,PaginatedDMAResponse,DeleteDMASingleRecord,DeleteRecordByAuditID,TotalDMARecordsResponse)
...
async def get_all_records(page:int=Query(1,ge=1,description="Page value should be greater than one"),page_size:int=Query(10,ge=1,le=100,description="The page value should be greater than one and less than 100"),session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
...
async def total_number_of_dma_records(session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
    total=await dma_manager.sum_number_of_dma_records(session)
    return TotalDMARecordsResponse(total_number_of_records=total)
...
async def get_dma_record_by_id(id:int=Path(...,description="Record id"),session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
...
async def delete_record_by_id(id:int=Path(...,description="Record id"),session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
...
async def delete_record_by_audit_id(audit_id:str=Path(...,description="Provide audit id for a database record"),session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
